...
function parseAdSource(rawUrl: string | null) {
  ...
  try {
    ...
    return {
      ...
      campaign_source:
        params.get("utm_campaign") || params.get("gad_campaignid") || null,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const finalNotes = [notes, additional_notes, message]
      .filter(Boolean)
      .join("\n\n");
    ...
    const uploadBase64File = async (
      ...
      if (!error) {
        const {
          data: { publicUrl },
        } = supabase.storage.from("submittal-pdfs").getPublicUrl(storageName);
        return publicUrl;
      }
      return null;
    };

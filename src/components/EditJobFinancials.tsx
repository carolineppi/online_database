...
export default function EditJobFinancials({ job, options: passedOptions, onClose }: { job: any, options?: any[], onClose: () => void }) {
  ...
  const [fetching, setFetching] = useState(!passedOptions); // True if options were not provided
  ...
  // If opened from the Jobs page (where options aren't passed), fetch them automatically
  useEffect(() => {
    if (!passedOptions) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-end z-50">
      <div className="bg-white w-full max-w-md h-full shadow-2xl flex flex-col animate-in slide-in-from-right">
        ...
        <div className="p-8 border-t border-zinc-100 bg-white shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
          ...
          <button 
            type="submit"
            form="financials-form" // Links button to the form above
